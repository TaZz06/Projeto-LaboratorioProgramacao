...
@section('content')
    ...
    <div class="flex flex-col w-full mt-8 -my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        <div class="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">
            <h3 class="pl-5 text-gray-700 text2xl font-medium">Companies Dashboard</h3>
            <table class="min-w-full">
                ...
                <tbody id="empresasDashboard"class="bg-white">
                    @foreach($infos as $info)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="text-sm leading-5 text-gray-900">{{$info->nif}}</div>
                            </td>

                            <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                                <form action="{{ route('remove_empresa', $info->empresas_id) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
